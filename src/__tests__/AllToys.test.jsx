...
describe('1st Deliverable', () => {
  test('Displays all toys on startup', async () => {
    ...
    const toyNames = toyCards.map(
      (toyCard) => toyCard.querySelector('h2').textContent
    )
    const baseToyNames = global.baseToys.map((toy) => toy.name)
    expect(toyNames).toEqual(baseToyNames)

    const toyImages = toyCards.map(
      (toyCard) => toyCard.querySelector('img').src
    )
    const baseToyImages = global.baseToys.map((toy) => toy.image)
    expect(toyImages).toEqual(baseToyImages)

    const toyLikes = toyCards.map(
      (toyCard) => toyCard.querySelector('p').textContent
    )
    const baseToyLikes = global.baseToys.map(
      (toy) => toy.likes.toString() + ' Likes '
    )
  ...
  test("Toys aren't hardcoded", async () => {
    ...
    const baseToyLikes = global.alternateToys.map(
      (toy) => toy.likes.toString() + ' Likes '
    )
    expect(toyLikes).toEqual(baseToyLikes)
  })
